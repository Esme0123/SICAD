...
import { getReemplazosAdmin, SolicitudReemplazo } from "@/services/reemplazos.service";

interface ReemplazosViewProps {
  dark: boolean;
}
...
const STATUS_MAP: Record<string, { label: string; color: string; darkColor: string }> = {
  ...
  ACEPTADO: { label: "Aceptado", color: "text-green-600", darkColor: "text-green-400" },
...
export const ReemplazosView: React.FC<ReemplazosViewProps> = ({ dark }) => {
  const [solicitudes, setSolicitudes] = useState<SolicitudReemplazo[]>([]);
  ...
  const loadData = async () => {
    try {
      const list = await getReemplazosAdmin();
      setSolicitudes(list);
    } catch (error) {
      console.error(error);
    }
  ...
  const filtered = useMemo(() => {
    return solicitudes.filter((s) => {
      ...
      const codigoS = s.solicitante?.codigo || "";
      const ciS = s.solicitante?.ci || "";
      const reemplazante = s.reemplazante?.nombre || "";
      const matchQ =
        !filterQ ||
        solicitante.toLowerCase().includes(q) ||
        reemplazante.toLowerCase().includes(q) ||
  ...
  }, [solicitudes, filterQ, filterFecha, filterEstado]);
        ...
        <div className={`flex flex-wrap items-center justify-between gap-4 p-5 border-b ${dark ? "border-white/8" : "border-slate-100"}`}>
          <div>
            ...
            <p className={`text-xs mt-0.5 ${dark ? "text-white/40" : "text-slate-400"}`}>
              Historial de solicitudes de reemplazo de bloques entre compañeros.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input
              ...
              placeholder="Buscar solicitante o reemplazante..."
          ...
          <table className="w-full">
            <thead>
              <tr className={dark ? "bg-white/3" : "bg-slate-50/80"}>
                {["Solicitante", "Reemplazante", "Fecha de Reemplazo", "Bloques / Horarios", "Comentario / Justificación", "Estado", "Acciones"].map((c) => (
              ...
              {filtered.length > 0 ? (
                  ...
                  <tr key={s.id} className={`border-t transition-colors ${dark ? "border-white/6 hover:bg-white/3" : "border-slate-100 hover:bg-primary/5"}`}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <Avatar name={s.solicitante?.nombre || ""} size={32} bg={COLORS.primary} />
                        <div>
                          <span className={`text-sm font-medium ${dark ? "text-white" : "text-slate-800"}`}>{s.solicitante?.nombre}</span>
                          <span className={`block text-xs font-mono ${dark ? "text-white/40" : "text-slate-400"}`}>{s.solicitante?.codigo}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      {s.reemplazante ? (
                        <div className="flex items-center gap-3">
                          <Avatar name={s.reemplazante.nombre} size={32} bg={COLORS.secondary} />
                          <div>
                            <span className={`text-sm font-medium ${dark ? "text-white" : "text-slate-800"}`}>{s.reemplazante.nombre}</span>
                            <span className={`block text-xs font-mono ${dark ? "text-white/40" : "text-slate-400"}`}>{s.reemplazante.codigo}</span>
                          </div>
                        </div>
                      ) : s.esAbierta ? (
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold ${dark ? "bg-sky-500/15 text-sky-300" : "bg-sky-50 text-sky-600"}`}>
                          <Users size={13} /> Petición Abierta
                        </span>
                      ) : (
                        <span className={`text-sm ${dark ? "text-white/40" : "text-slate-400"}`}>—</span>
                      )}
                    </td>
                    ...
                    <td className={`px-5 py-4 text-sm max-w-[220px] truncate ${dark ? "text-white/50" : "text-slate-500"}`}>{s.comentario || "—"}</td>
                ...
                <tr>
                  <td colSpan={7} className={`px-5 py-12 text-center text-sm ${dark ? "text-white/40" : "text-slate-500"}`}>
                    No se encontraron solicitudes de reemplazo.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        >
          <div className={`w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh] ${dark ? "bg-[#1E293B] border border-white/10" : "bg-white"}`}>
            <div className={`flex items-center justify-between px-6 py-4 border-b flex-shrink-0 ${dark ? "border-white/10" : "border-slate-100"}`}>
              <h3 className={`text-lg font-bold ${dark ? "text-white" : "text-slate-800"}`}>Detalle del Reemplazo</h3>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-base font-bold ${dark ? "text-white" : "text-slate-800"}`}>
                    {detail.esAbierta ? "Petición Abierta" : "Petición a Empleado"}
                  </p>
                  <p className={`text-xs mt-0.5 ${dark ? "text-white/50" : "text-slate-400"}`}>ID #{detail.id}</p>
                </div>
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                  detail.estado === "ACEPTADO"
                    ? "text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-900/30"
                    : detail.estado === "RECHAZADO"
                      ? "text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-900/30"
                      : "text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30"
                }`}>
                  {detail.estado === "ACEPTADO" ? <CheckCircle2 size={14} /> : detail.estado === "RECHAZADO" ? <XCircle size={14} /> : <AlertCircle size={14} />}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className={`flex items-center gap-3 p-3 rounded-xl border ${dark ? "bg-white/5 border-white/10" : "bg-slate-50 border-slate-200"}`}>
                  <Avatar name={detail.solicitante?.nombre || ""} size={34} bg={COLORS.primary} />
                  <div className="min-w-0">
                    <p className={`text-[10px] font-bold uppercase ${dark ? "text-white/40" : "text-slate-400"}`}>Solicitante</p>
                    <p className={`text-sm font-semibold truncate ${dark ? "text-white" : "text-slate-800"}`}>{detail.solicitante?.nombre}</p>
                    <p className={`text-xs ${dark ? "text-white/50" : "text-slate-500"}`}>{detail.solicitante?.codigo || ""}</p>
                  </div>
                </div>
                <div className={`flex items-center gap-3 p-3 rounded-xl border ${dark ? "bg-white/5 border-white/10" : "bg-slate-50 border-slate-200"}`}>
                  {detail.esAbierta ? <Users size={22} className="text-sky-500 shrink-0" /> : <Repeat size={22} className="text-sky-500 shrink-0" />}
                  <div className="min-w-0">
                    <p className={`text-[10px] font-bold uppercase ${dark ? "text-white/40" : "text-slate-400"}`}>Reemplazante</p>
                    <p className={`text-sm font-semibold truncate ${dark ? "text-white" : "text-slate-800"}`}>
                      {detail.reemplazante?.nombre || (detail.esAbierta ? "Petición abierta" : "Pendiente")}
                    </p>
                    <p className={`text-xs ${dark ? "text-white/50" : "text-slate-500"}`}>{detail.reemplazante?.codigo || ""}</p>
                  </div>
                </div>
              </div>

              <div>
                <p className={`text-xs font-medium mb-1 ${dark ? "text-white/50" : "text-slate-400"}`}>Fecha de Reemplazo</p>
              ...
              <div>
                <p className={`text-xs font-medium mb-1.5 ${dark ? "text-white/50" : "text-slate-400"}`}>
                  Bloques a Reemplazar ({(detail.bloques as unknown[]).length})
                </p>
                <div className="flex flex-wrap gap-2">
                  {(detail.bloques as Array<{ nombre?: string; horaInicio?: string; horaFin?: string; duracion?: number }>).map((b, i) => (
              ...
              <div>
                <p className={`text-xs font-medium mb-1 ${dark ? "text-white/50" : "text-slate-400"}`}>Comentario / Justificación</p>
                <div className={`p-3 rounded-lg text-sm ${dark ? "bg-white/5 text-white/70 border border-white/10" : "bg-slate-50 text-slate-600 border border-slate-200"}`}>
                  {detail.comentario || <span className="italic opacity-60">Sin comentario</span>}
                </div>
              </div>
