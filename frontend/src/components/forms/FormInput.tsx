...
export const FormInput = forwardRef<HTMLInputElement, FormInputProps>(
  ({ label, error, icon, dark, className = "", ...props }, ref) => {
    ...
    const inputCls = `w-full py-3 rounded-xl border text-sm outline-none transition-all ${
      dark
        ? "bg-white/5 border-white/10 text-white placeholder-white/25 focus:border-[var(--color-primary)]/60"
        : "bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-[var(--color-primary)]/50"
